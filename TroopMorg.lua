...
function AutoUpdate(data)
    if tonumber(data) > tonumber(ver) then
        ...
        DownloadFileAsync("https://raw.githubusercontent.com/TrooperHDxLeagueSharp/GoS/master/TroopMorg.lua", SCRIPT_PATH .. "TroopMorg.lua", function() print("Update Complete, please 2x F6!") return end)
    end
end
...
TroopMorg:SubMenu("AutoE", "Auto E for cc")
TroopMorg.AutoE:Boolean("CC", "Use E on CC", true)
	...
	if Mode() == "Harass" then
		...
		if TroopMorg.Harass.WHarass:Value() and Ready(_W) and ValidTarget(target, 900) then
			...
			if TroopMorg.Harass.MinManaHarass:Value() <= GetPercentMana then
				...
				if Qpredd.hitChance >= (TroopMorg.Prediction.Q:Value() * 0.01) and not Qpredd:mCollision(1) then
